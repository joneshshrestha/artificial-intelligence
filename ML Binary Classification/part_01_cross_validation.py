...
def custom_metric_print(metrics_dict):
    ...
    for dataset_type in ['train', 'validation']:
        ...
        for class_name in sorted(dataset_metrics.keys()):
            if class_name not in ['accuracy', 'macro avg', 'weighted avg']:
                ...
                print(f"{dataset_label:<12} {class_name:<12} "
    ...
    if 'train' in metrics_dict and 'validation' in metrics_dict:
        ...
        print(f"{'Accuracy':<20} {train_acc:<15.4f} {val_acc:<15.4f} {train_acc - val_acc:<15.4f}")
        
        for avg_type in ['macro avg', 'weighted avg']:
            if avg_type in metrics_dict['train'] and avg_type in metrics_dict['validation']:
                for metric in ['precision', 'recall', 'f1-score']:
                    ...
                    print(f"{metric_name:<20} {train_val:<15.4f} {val_val:<15.4f} {train_val - val_val:<15.4f}")
...
def cross_validation(raw_x: np.ndarray, raw_y: np.ndarray, n_folds: int, classifier_name: str, hyper_params: Dict) -> Dict:
    ...
    for i in range(n_folds):
        ...
        train_report = classification_report(train_y, train_predictions, output_dict=True)
    ...
    for dataset_type, classification_reports_list in metrics_data.items():
        ...
        for key in first_classification_report.keys():
            if key == 'accuracy':
                ...
                sub_metrics_dict[key] = np.mean([report[key] for report in classification_reports_list])
            else:
                ...
                for metric in first_classification_report[key].keys():
                    ...
                    if metric == 'support':
                        group_metrics[metric] = np.sum([report[key][metric] for report in classification_reports_list])
                    # average precision, recall, f1-score across all folds
                    else:
                        group_metrics[metric] = np.mean([report[key][metric] for report in classification_reports_list])
    ...
    for class_name in sorted(final_metrics['train'].keys()):
        if class_name not in ['accuracy', 'macro avg', 'weighted avg']:
            print(f"{class_name}: precision={final_metrics['train'][class_name]['precision']:.4f}, "
    ...
    for class_name in sorted(final_metrics['validation'].keys()):
        if class_name not in ['accuracy', 'macro avg', 'weighted avg']:
            print(f"{class_name}: precision={final_metrics['validation'][class_name]['precision']:.4f}, "
...
def main():
    ...
    # 1) Load your (cross-validation) hyper-parameters
    ...
    print(f"Dataset loaded: {dataset_x.shape[0]} samples, {dataset_x.shape[1]} features")
