...
def main():
    if len(sys.argv) < 5:
        print("Usage:")
        print(
            f"\tpython {sys.argv[0]} in_config in_raw_data out_normalizer out_classifier"
        )
        return
    ...
    print(
        f"Macro Avg - Precision: {train_report['macro avg']['precision']:.4f}, "
        f"Recall: {train_report['macro avg']['recall']:.4f}, "
        f"F1-Score: {train_report['macro avg']['f1-score']:.4f}"
    )
    print(
        f"Weighted Avg - Precision: {train_report['weighted avg']['precision']:.4f}, "
